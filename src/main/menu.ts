...
import { instantUpsCmd, reloadUpsdConf as reloadUpsdConf, restartProcesses } from './cmder';
...
export const createMenuFull = ({
  ...
  return Menu.buildFromTemplate([
      ...
      submenu: [
        {
          ...
          enabled: upsData?.get('outlet.1.status') ? upsData?.get('outlet.1.status').toString().includes('off'): false,
          click: async () => await instantUpsCmd(UPS_CMD.OUTLET_1_LOAD_ON),
        },
        {
          label: `Turn off the load on outlet 1 immediately`,
          enabled: upsData?.get('outlet.1.status') ? upsData?.get('outlet.1.status').toString().includes('on') : false,
      ...
      submenu: [
        {
          ...
          enabled: upsData?.get('outlet.2.status') ? upsData?.get('outlet.2.status').toString().includes('off') : false,
          click: async () => await instantUpsCmd(UPS_CMD.OUTLET_2_LOAD_ON),
        },
        {
          label: `Turn off the load on outlet 2 immediately`,
          enabled: upsData?.get('outlet.2.status') ? upsData?.get('outlet.2.status').toString().includes('on'): false,
    ...
    {
      label: 'Force restart upsd and driver',
      click: () => restartProcesses(),
    },
    {
      label: 'Reload upsd configuration',
