...
const getUpsTrayStatusTitle = (data: UPS_DATA) => {
...
export function getTitleOnStatus(data: UPS_DATA, isCompactView = true): string {
  const upsStatusTray = getUpsTrayStatusTitle(data);
  const upsStatus = convertStatus((data?.get('ups.status') as string) || null);

  const temp = isCompactView ? 'T:' : 'Temp:';
  const load = isCompactView ? 'L:' : 'Load:';
  const batLevel = store.get('batPercentageTray') ? `${data?.get('battery.charge')}% | ` : ''

  if (upsStatus.includes(UPS_STATUS.OB)) {
    return ` ${batLevel}${upsStatusTray} | ${temp} ${data?.get('ambient.temperature')}° | ${load} ${data?.get('ups.load')}%`;
  } else {
