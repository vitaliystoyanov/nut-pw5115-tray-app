...
import { parseUpsData, UPS_DATA } from 'parser/upsData';
...
const NUT_UPS_INSTANT_CMD = `${NUT_BIN_PATH}/upscmd -u admin -p admin `;
const NUT_UPSC_CMD = `${NUT_BIN_PATH}/upsc ${UPS_NAME}`;
...
export async function instantUpsCmd(cmd: UPS_CMD, upsName?: string) {
  let return_;
  try {
    return_ = await exec(
      `${NUT_UPS_INSTANT_CMD} ${upsName ? upsName : UPS_NAME} ${cmd}`,
    );
    return { cmdResult: return_, status: CMD_RUN_STATUS.OK };
  } catch (err) {
    return { cmdResult: return_, status: CMD_RUN_STATUS.ERROR };
  }
}
...
export async function requestUpsData(): Promise<UPS_DATA | null> {
  try {
    const exec = util.promisify(childProcess.exec);
    const result = await exec(NUT_UPSC_CMD);
    return transform(parseUpsData(result.stdout));
  } catch (err) {
    return null;
  }
}
